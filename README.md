...
纯前端单页面工具，用来把 ChatGPT Web 登录 session JSON 或 xAI OAuth JSON 转换成 CPA、sub2api、Cockpit Tools、9router、Codex auth.json、AxonHub、Codex-Manager 或 Grok2API 可导入 JSON。
...
也支持粘贴或拖入 xAI OAuth JSON，例如包含 `type: "xai"`、`auth_kind: "oauth"`、`access_token`、`refresh_token`、`expired` 和 `email`。
...
- `Grok2API`：将一个 xAI OAuth JSON 生成 Grok2API 的 `{ "accounts": [...] }` 导入包。账号字段匹配 `grok_build` 格式：从 access token 的 JWT claims 读取 `client_id` 和 `sub`，保留 access/refresh token，并将 `id_token`、`scope`、`principal_id`、`team_id` 输出为空字符串以匹配示例格式。
